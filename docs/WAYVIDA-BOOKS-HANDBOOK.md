...
The current branch is `main` with committed baseline `632e747`. The field-first Inventory Adjustment create-page change and its synchronized tests/docs are current working-tree work. Another agent must inspect and preserve the working tree before editing. The latest verified commands are the Inventory Adjustment suite at 25/25, the documentation validator, the production Vite build (6,959 modules; existing large-chunk advisory only), Sites preparation and a read-only live-browser inspection at port 4001.

...
export const money=(n,format)=>formatMinor(n,format);
...
/* The next free invoice number. Counting the records is not enough: a filtered or partially loaded
   list would hand the same number to two documents, and the uniqueness check would then reject a
   save with a confusing error. It scans for the first number that is not already used. */
export function nextInvoiceNumber(invoices=[]){const used=new Set((invoices||[]).map(row=>String(row.number||'').trim().toUpperCase()));let n=1;while(used.has('INV-'+String(n).padStart(4,'0')))n++;return 'INV-'+String(n).padStart(4,'0')}
...
const RESERVED_POSTING_ACCOUNTS={
 '1010':{name:'Bank',type:'Assets',nature:'Debit',group:'Cash and Bank',controlAccount:false},
 '1100':{name:'Accounts Receivable',type:'Assets',nature:'Debit',group:'Current Assets',controlAccount:true},
 '2000':{name:'Accounts Payable',type:'Liabilities',nature:'Credit',group:'Current Liabilities',controlAccount:true},
...
export function account(s,code,type){
 ...
 /* A journal line carries only an account code, so a reserved system ledger is
    repaired whether or not the caller named the type it expects. The bank ledger
    is a hardcoded money account in the receipt, purchase, banking and demo-seed
    paths, and without this a deleted or deactivated ledger made every later load
    report "Wayvida Books could not start." */
 if(reserved&&(!type||type.includes(reserved.type))&&(!a||!a.active||a.isGroup||a.type!==reserved.type)){
 ...
 if(!a?.active||a.isGroup||type&&!type.includes(a.type))fail('Configure an active posting '+(type?.join('/')||a?.type||'ledger')+' account for '+(code||'missing mapping')+'.');return code
}
...
/* Who may do what to a legal document. This mirrors JOURNAL_PERMISSIONS in
   src/simple-journal-transaction.js and RECEIPT_ROLES in src/receipt-engine.js, which both draw the
   boundary at the command rather than by hiding buttons: an accountant may raise and submit an
...
export function command(state,action,payload,ctx={}){
 if(ctx.role&&!invoiceAllowed(ctx.role,action))fail('Your role cannot '+action+' an invoice. Ask a finance manager or admin.');
 const s=clone(state),p=clone(payload);let i=s.invoices.find(x=>x.id===p.id),result;const wasStatus=i?i.status:'';
 if(action==='save'){
  ...
  /* The invoice carries its own organisation and branch: posting stamps the journal from them, so an
     invoice without a scope must not be saved at all. Enforced when a role is supplied, so the
     engine's role-less contract is unchanged. */
  if(ctx.role&&!p.organizationId)fail('Select the organisation this invoice is raised in.');
  if(ctx.role&&!p.branchId)fail('Select the branch this invoice is raised in.');
  const number=p.number?.trim()||nextInvoiceNumber(s.invoices);if(s.invoices.some(x=>x.number.toLowerCase()===number.toLowerCase()&&x.id!==p.id))fail('Invoice number already exists.');const totals=calculate(p,s.config);result={...p,id:i?.id||crypto.randomUUID(),number,totals,revision:(i?.revision||0)+1,taxPolicy:{state:s.config.state,roundRupee:s.config.roundRupee},status:'Draft',posted:false,createdAt:i?.createdAt||new Date().toISOString()};s.invoices=i?s.invoices.map(x=>x.id===i.id?result:x):[...s.invoices,result];
 }else if(action==='config'){Object.assign(s.config,p);result=s.config;
 }else{
  if(!i)fail('Invoice not found.');result=i;
  if(action==='submit'){if(i.status!=='Draft')fail('Only drafts may be submitted.');if(ctx.role&&!i.organizationId)fail('Select the organisation this invoice is raised in.');if(ctx.role&&!i.branchId)fail('Select the branch this invoice is raised in.');i.status='Pending Approval';}
  else if(action==='post'){
   if(i.posted)return {state:s,result:i};
   /* Only an approved document becomes a posting. The role-less call keeps the older contract, so
      the engine remains usable by tests and importers. */
   if(ctx.role&&!i.organizationId)fail('This invoice has no organisation. Set the scope before posting.');
   if(ctx.role&&!i.branchId)fail('This invoice has no branch. Set the scope before posting.');
   if(ctx.role&&i.status!=='Pending Approval')fail('Submit the invoice for approval before posting it.');
   if(!['Draft','Pending Approval'].includes(i.status))fail('Invoice cannot be posted.');const t=calculate(i,i.taxPolicy||s.config);if(t.total<=0)fail('Invoice total must be positive.');validateAccountTaxes(s,t);const ar=account(s,i.receivableAccount||s.config.ar,['Assets']);const lines=[{account:ar,debit:t.total,credit:0}];t.lines.forEach(l=>lines.push({account:account(s,l.income||s.config.sales,['Income']),debit:0,credit:l.taxable}));for(const k of ['cgst','sgst','igst','cess'])if(t[k])lines.push({account:account(s,s.config[k],['Liabilities']),debit:0,credit:t[k],description:k.toUpperCase()});if(t.roundOff)lines.push({account:account(s,s.config.round,['Expenses']),debit:Math.max(-t.roundOff,0),credit:Math.max(t.roundOff,0)});const j=journal(s,i,'Sales Invoice',lines,i.date,'invoice:'+i.id);Object.assign(i,{posted:true,status:'Approved',totals:t,journalId:j.id,arAccount:ar,postedAt:j.createdAt});
  }else if(action==='send'){if(!i.posted||i.status==='Cancelled')fail('Post the invoice first.');i.status='Sent';}
  else if(action==='pay'){
   if(s.payments.some(x=>x.token===p.token))return {state:s,result:i};if(!p.token)fail('Payment request ID required.');if(!i.posted||i.status==='Cancelled')fail('Only posted invoices can receive payments.');if(!dateOK(p.date)||p.date<i.date)fail('Payment date must be on or after invoice date.');const amount=minor(p.amount);if(amount<=0||amount>outstanding(s,i))fail('Payment must be positive and cannot exceed outstanding.');const bank=s.accounts.find(a=>a.code===p.bank&&a.active&&a.type==='Assets'&&/bank|cash|wallet/i.test([a.name,a.group,a.accountNature,a.accountPurpose].filter(Boolean).join(' ')));if(!bank)fail('Select an active cash, bank or wallet account.');const j=journal(s,i,'Customer Payment',[{account:bank.code,debit:amount,credit:0},{account:i.arAccount,debit:0,credit:amount}],p.date,'payment:'+p.token);s.payments.push({id:crypto.randomUUID(),number:'PAY-'+String(s.payments.length+1).padStart(4,'0'),invoiceId:i.id,customerId:i.customerId,amount,date:p.date,bank:p.bank,reference:p.reference||'',journalId:j.id,token:p.token});
 ...
 s.audit.push({id:crypto.randomUUID(),action,invoiceId:result.id||null,at:new Date().toISOString(),by:ctx.actor||ctx.role||'Admin',from:wasStatus,to:result.status,journalId:result.journalId||null,reason:p.reason||''});return {state:s,result};
}

...
import {CUSTOMER_CREATE_KEY} from './quick-create.js';
...
/* ONE rule decides the customer type everywhere. A stored row can predate the field, and the master
   has always treated anything that is not an individual as a business, so the sales create pages read
   the same answer this form shows instead of inventing a second default. */
export const customerTypeOf=row=>row?.type==='Individual'?'Individual':'Business';
...
/* A sales document can also send the operator to a NEW customer: the same handoff with no id, which opens
   a blank form. An edit marker wins if both were somehow set. */
useEffect(()=>{try{if(sessionStorage.getItem('wayvida-edit-customer'))return;if(!sessionStorage.getItem(CUSTOMER_CREATE_KEY))return;sessionStorage.removeItem(CUSTOMER_CREATE_KEY);start()}catch{}},[]);
   ...
   <div className="customerDetailBody">
    <section className="customerDetailPanel">
     <div className="customerDetailIdentity">
      ...
      <div className="customerDetailName"><div className="customerNameRow"><h2>{selected.name}</h2><StatusPill status={selected.status||'Active'} tone={selected.status==='Inactive'?'neutral':'ok'}/></div><p>{selected.code||'No customer code'} · {selected.type||'Business'} · {Number(selected.days||0)} credit days</p></div>

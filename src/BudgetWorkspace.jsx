...
import {IconArrowLeft,IconCheck,IconChevronDown,IconChevronRight,IconPlus,IconSearch,IconTrash,IconEdit,IconCopy,IconDownload,IconArchive,IconChartBar,IconSettings,IconAlertTriangle,IconEye,IconFileImport,IconFileExport,IconUpload,IconRefresh,IconDeviceFloppy,IconSend,IconCircleCheck,IconPlayerPlay,IconChecklist,IconClock,IconCalendar,IconTrendingUp,IconTrendingDown,IconX,IconDots,IconInfoCircle} from '@tabler/icons-react';
...
/* The one date reading the budget screens print, matching the journal and
   Period Closing date style: 01 Apr 2026. */
...
function BudgetList({state,business,create,view,viewStatement,edit,duplicate,exportBudget,archive,remove,importFile}){
  ...
  const advancedDefaults={type:'All types',period:'All periods',scope:'All scopes',status:'All statuses'};
  const activeFilterCount=['type','period','scope','status'].filter(key=>filters[key]!==advancedDefaults[key]).length;
  const clearAdvanced=()=>setFilters({...filters,...advancedDefaults});
  return <section className="budgetV3">
    <div className="budgetV3Head"><div><h1>{business?'Budgets':'Budget Management'}</h1><p>Plan, monitor and control financial performance.</p></div><div className="budgetHeadActions"><button onClick={()=>importRef.current?.click()}><IconFileImport size={17}/>Import</button><button className="primary" onClick={create}><IconPlus size={17}/>Create Budget</button><input ref={importRef} type="file" accept=".csv,text/csv" hidden onChange={event=>{if(event.target.files?.[0])importFile(event.target.files[0]);event.target.value=''}}/></div></div>
    <section className="budgetListCard">
      <div className="budgetRegisterBar"><label className="budgetFilterSearch"><IconSearch size={17}/><input value={filters.query} onChange={event=>setFilters({...filters,query:event.target.value})} placeholder="Search budget or financial year" aria-label="Search budgets"/></label><select className="budgetFilterYear" aria-label="Financial year" value={filters.year} onChange={event=>setFilters({...filters,year:event.target.value})}><option>All years</option>{FINANCIAL_YEARS.map(value=><option key={value}>{value}</option>)}</select><details className="budgetFiltersMore"><summary aria-label="Open filters">Filters{activeFilterCount?<em className="budgetFilterBadge">{activeFilterCount}</em>:null}</summary><div className="budgetFiltersPanel"><label className="budgetFilterField"><span>Budget type</span><select value={filters.type} onChange={event=>setFilters({...filters,type:event.target.value})}><option>All types</option>{BUDGET_TYPES.map(type=><option key={type.id}>{type.id}</option>)}</select></label><label className="budgetFilterField"><span>Budget period</span><select value={filters.period} onChange={event=>setFilters({...filters,period:event.target.value})}><option>All periods</option>{BUDGET_PERIODS.map(value=><option key={value}>{value}</option>)}</select></label><label className="budgetFilterField"><span>Scope</span><select value={filters.scope} onChange={event=>setFilters({...filters,scope:event.target.value})}><option>All scopes</option>{BUDGET_SCOPES.map(value=><option key={value}>{value}</option>)}</select></label><label className="budgetFilterField"><span>Status</span><select value={filters.status} onChange={event=>setFilters({...filters,status:event.target.value})}><option>All statuses</option>{['Draft','Pending Approval','Approved','Active','Completed','Archived'].map(value=><option key={value}>{value}</option>)}</select></label>{activeFilterCount?<div className="budgetFiltersPanelActions"><button type="button" onClick={clearAdvanced}>Clear filters</button></div>:null}</div></details></div>
      <table className="budgetTable"><thead><tr><th scope="col">Budget Name</th><th scope="col">Financial Year</th><th scope="col">Budget Period</th><th scope="col">Actions</th></tr></thead><tbody>{list.map(budget=><tr key={budget.id}><td><b>{budget.name}</b></td><td>{budget.financialYear}</td><td>{budget.period}</td><td className="budgetActionsCell"><div className="budgetRowActions"><button type="button" className="budgetRowView" aria-label={`View Details for ${budget.name}`} onClick={()=>view(budget)}><IconEye size={15}/>View Details</button><details className="budgetKebab"><summary aria-label={`More actions for ${budget.name}`}><IconDots size={17}/></summary><div className="budgetKebabMenu"><button type="button" onClick={menuRun(()=>viewStatement(budget))}><IconEye size={16}/>View budget</button><button type="button" onClick={menuRun(()=>edit(budget))}><IconEdit size={16}/>Edit</button><button type="button" onClick={menuRun(()=>duplicate(budget))}><IconCopy size={16}/>Duplicate</button><button type="button" className="budgetKebabDanger" onClick={menuRun(()=>remove(budget))}><IconTrash size={16}/>Delete</button></div></details></div></td></tr>)}{!list.length&&<tr><td colSpan="4" className="budgetEmpty">No budgets match these filters. Create a budget to start planning.</td></tr>}</tbody></table>
    </section>
  </section>;
}
    ...
    {step===3&&<section className="budgetAllocation">
      <div className="budgetSelectHead"><div><h2>Budget allocation</h2><p>Set planned amounts for each account and period. Every cell stays editable, and the shortcuts fill the grid for you.</p></div>
        <div className="budgetAllocationActions">
          ...
          <label className="budgetImportButton"><IconFileImport size={16}/>Import Budget<input type="file" accept=".csv,text/csv" onChange={importFile}/></label>

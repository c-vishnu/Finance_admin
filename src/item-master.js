...
export const blankItem=(defaults={})=>({name:'',type:'Goods',unit:'pcs',sku:'',category:'General',hsnSac:'',image:'',sales:true,purchase:true,price:'',cost:'',salesAccount:defaults.salesAccount||'',purchaseAccount:defaults.purchaseAccount||'',trackInventory:true,inventoryAccount:defaults.inventoryAccount||'',cogsAccount:defaults.cogsAccount||'',openingQuantity:'',openingRate:'',openingDate:'2026-09-05',warehouseId:defaults.warehouseId||'',taxPreference:'Taxable',taxApplicable:true,taxRate:String(defaults.taxRate||'18'),cessRate:'0',priceTaxMode:'exclusive',organizationId:defaults.organizationId||'abc',currency:defaults.currency||'INR'});
...
export function validateItem(form,items,accounts){
 ...
 if(form.taxPreference==='Taxable'&&(!Number.isFinite(Number(form.taxRate))||Number(form.taxRate)<0||Number(form.taxRate)>100))errors.taxRate='Select a valid GST rate.';
 return errors;
}

...
export const blankItem=(defaults={})=>({name:'',type:'Goods',unit:'pcs',sku:'',category:'General',hsnSac:'',description:'',image:'',sales:false,purchase:false,price:'',cost:'',salesAccount:defaults.salesAccount||'',purchaseAccount:defaults.purchaseAccount||'',trackInventory:false,inventoryAccount:defaults.inventoryAccount||'',cogsAccount:defaults.cogsAccount||'',openingQuantity:'',openingRate:'',openingDate:'2026-09-05',warehouseId:defaults.warehouseId||'',taxPreference:'Non-taxable',taxApplicable:false,taxRate:String(defaults.taxRate||'18'),interStateTaxRate:String(defaults.interStateTaxRate||defaults.taxRate||'18'),cessRate:'0',priceTaxMode:'exclusive',organizationId:defaults.organizationId||'abc',organizationIds:defaults.organizationIds||[defaults.organizationId||'abc'],branchIds:defaults.branchIds||(defaults.warehouseId?[defaults.warehouseId]:[]),active:true,auditTrail:[],currency:defaults.currency||'INR'});
export const itemOrganisationIds=item=>Array.from(new Set((item?.organizationIds?.length?item.organizationIds:[item?.organizationId]).filter(Boolean)));
...
export function validateItem(form,items,accounts){
 ...
 const scope=new Set(itemOrganisationIds(form));
 const overlaps=item=>itemOrganisationIds(item).some(id=>scope.has(id));
 if(items.some(item=>item.id!==form.id&&overlaps(item)&&item.name.toLowerCase()===form.name.trim().toLowerCase()))errors.name='An item with this name already exists in this organisation.';

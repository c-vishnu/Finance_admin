...
import {ITEM_CREATE_KEY} from './quick-create.js';
...
export default function Items({accounts,seed:accountSeed,notify}){
 ...
 /* A sales document can send the operator here to set an item up mid-document. The marker is read
    once and cleared, so the create form opens on the arrival that asked for it and never again. */
 useEffect(()=>{let pending=false;try{pending=sessionStorage.getItem(ITEM_CREATE_KEY)!==null;if(pending)sessionStorage.removeItem(ITEM_CREATE_KEY)}catch{}if(pending)start()},[]);
 /* Every hook sits above the detail-page early return. A hook below it ran on the register and not
    on the detail, which is the "Rendered fewer hooks than expected" crash the item detail threw. */
 const importRef=useRef(null);
 /* Item prices are stored in rupees, not paise, and this screen formats them through the shared helper,
    so the register and the item detail follow the Number and Currency Format and re-render the moment it
    changes. The formatter itself is declared with the other derived values below. */
 const numberFormat=useNumberFormat();
 const changeType=type=>setForm(current=>({...current,type,unit:type==='Service'?'hour':'pcs',hsnSac:'',...itemDefaults(accounts,type)}));
        ...
        <section className="itemDetailPanel">
          <div className="itemDetailIdentity">
            ...
            <div className="itemDetailName">
              <div className="itemDetailNameRow"><h2>{selected.name}</h2><span className={'itemStatus '+(selected.active===false?'disabled':'active')}>{selected.active===false?'Disabled':'Active'}</span></div>
 ...
 <Section><label className="itemCheck"><input type="checkbox" role="switch" checked={form.purchase} onChange={e=>set('purchase',e.target.checked)}/><span><b>Purchase this item</b><small>Show it in purchase bills and expenses.</small></span></label>{form.purchase&&<div className="itemFormGrid">{priceField(`Purchase price (${form.currency})`,'cost')}{accountField('Purchase Account','Expenses','purchaseAccount',form.type==='Service'?/professional|service|expense/i:/cost of goods|purchase|direct/i)}</div>}</Section>
  <div className="itemAdvanced"><div className="itemAdvancedGroup"><div className="itemAdvancedTitle">Tax settings</div><div className="itemTaxHead"><span className="itemTaxLabel">Tax treatment</span><div className="itemSegmented" role="group" aria-label="Tax treatment">{['Taxable','Non-taxable'].map(value=><button type="button" key={value} className={isTaxable===(value==='Taxable')?'active':''} aria-pressed={isTaxable===(value==='Taxable')} onClick={()=>setTaxable(value==='Taxable')}>{value}</button>)}</div>{!isTaxable&&<span className="itemTaxHint">This item is not subject to GST.</span>}</div>{isTaxable&&<><div className="itemFormGrid itemTaxFormGrid"><label>Intra-State Tax Rate *<select value={form.taxRate} onChange={e=>set('taxRate',e.target.value)} aria-invalid={!!errors.taxRate}>{ITEM_TAX_RATES.map(rate=><option key={rate} value={rate}>{rate}%</option>)}</select>{errors.taxRate&&<small className="itemError">{errors.taxRate}</small>}</label><label>Inter-State Tax Rate *<select value={form.interStateTaxRate} onChange={e=>set('interStateTaxRate',e.target.value)} aria-invalid={!!errors.interStateTaxRate}>{ITEM_TAX_RATES.map(rate=><option key={rate} value={rate}>{rate}%</option>)}</select>{errors.interStateTaxRate&&<small className="itemError">{errors.interStateTaxRate}</small>}</label><label>Cess rate<select value={form.cessRate} onChange={e=>set('cessRate',e.target.value)}>{ITEM_CESS_RATES.map(rate=><option key={rate} value={rate}>{rate}%</option>)}</select></label><label>Price type *<select value={form.priceTaxMode} onChange={e=>set('priceTaxMode',e.target.value)}>{!form.priceTaxMode&&<option value="">Select price type</option>}<option value="exclusive">Tax Exclusive</option><option value="inclusive">Tax Inclusive</option></select>{errors.priceTaxMode&&<small className="itemError">{errors.priceTaxMode}</small>}</label></div><p className="itemTaxNote">Intra-state transactions use CGST + SGST. Inter-state transactions use IGST.</p><dl className="itemTaxPreview" aria-label="Tax split preview"><div><dt>CGST</dt><dd>{gstPercent(taxSplit.cgst)}</dd></div><div><dt>SGST</dt><dd>{gstPercent(taxSplit.sgst)}</dd></div><div><dt>IGST</dt><dd>{gstPercent(interTaxSplit.igst)}</dd></div></dl></>}</div><div className="itemAdvancedGroup"><div className="itemAdvancedTitle">Item image</div><div className="itemImageField"><label className="itemImageUpload"><input type="file" accept="image/png,image/jpeg,image/webp" onChange={upload}/><span className="itemImageIcon"><IconPhotoPlus size={20}/></span><span className="itemImageCopy"><b>{form.image?'Replace image':'Choose image'}</b><small>JPG, PNG or WebP · Maximum 2 MB</small></span><span className="itemImageChoose">Browse</span></label>{form.image&&<div className="itemPreview"><img src={form.image} alt="Item preview"/><span><b>Image ready</b><small>Saved when you create the item.</small></span><button type="button" onClick={()=>set('image','')}>Remove</button></div>}{errors.image&&<small className="itemError">{errors.image}</small>}</div></div></div>{errors.save&&<p role="alert" className="itemError itemSaveError">{errors.save}</p>}</div><footer className="itemDialogFooter"><button type="button" onClick={()=>setForm(null)}>Cancel</button><button className="primary" disabled={uploadBusy}>{uploadBusy?'Processing image…':form.id?'Save changes':'Create Item'}</button></footer></form>}</section>;
}

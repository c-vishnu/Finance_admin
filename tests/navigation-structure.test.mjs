...
test('sidebar destinations are grouped once without duplicate routes',()=>{
  ...
  assert.ok(navigation.includes("{label:'Inventory',icon:IconPackage,children:[leaf('Items')]}"));
...
test('Accounting holds one Budgets destination between Journal Entries and Period Closing',()=>{
  ...
  assert.ok(navigation.includes("{label:'Inventory',icon:IconPackage,children:[leaf('Items')]}"),'Inventory stays a top-level section of its own');
});

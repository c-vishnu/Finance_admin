...
test('valid vendor can be linked to Accounts Payable without a vendor GL account',()=>{
  const vendor={...vendorDefaults,name:'Wayvida',phone:'9876543210',payableAccountId:'2100'};
  assert.deepEqual(validateVendor(vendor,[]),{});
});
test('GST and duplicate vendor validation protects the master',()=>{
  const vendor={...vendorDefaults,name:'Wayvida',phone:'9876543210',gstRegistered:true,gstin:'INVALID',payableAccountId:'2100'};
  const errors=validateVendor(vendor,[{id:'other',name:'wayvida'}]);

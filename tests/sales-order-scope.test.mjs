...
test('an item set up for the chosen organisation and branch is accepted',()=>{
 assert.equal(itemScopeError(good(),{organisationRefs:['abc','ABC01'],branchRefs:['abc-kochi','Kochi Branch'],organisationName:'ABC Technologies Pvt Ltd',branchName:'Kochi Branch'}),'');
});

test('the organisation code the Item scope picker writes is accepted as well as the id',()=>{
 assert.equal(itemScopeError(good({organizationIds:['ABC01'],organizationId:'ABC01'}),{organisationRefs:['abc','ABC01'],branchRefs:['abc-kochi'],organisationName:'ABC Technologies Pvt Ltd'}),'');
});

test('an item that belongs to another organisation is named on the line',()=>{
 const message=itemScopeError(good({organizationIds:['northstar']}),{organisationRefs:['abc','ABC01'],organisationName:'ABC Technologies Pvt Ltd'});
 assert.match(message,/Office stationery box is not set up for ABC Technologies Pvt Ltd in the Item master\./);
});

test('a stocked good that lives in another branch is named on the line',()=>{
 const message=itemScopeError(good({branchIds:['abc-bengaluru'],warehouseId:'abc-bengaluru'}),{organisationRefs:['abc'],branchRefs:['abc-kochi'],organisationName:'ABC Technologies Pvt Ltd',branchName:'Kochi Branch'});
...
test('an item with no organisation scope recorded is not blocked',()=>{
 assert.equal(itemScopeError({id:'seed',name:'Legacy item',type:'Goods',unit:'pcs'},{organisationRefs:['abc'],organisationName:'ABC Technologies Pvt Ltd'}),'');
});
...
test('the order report names the first line that cannot be fulfilled',()=>{
 ...
 assert.equal(orderScopeError([{item:'item-2'},{item:'item-1'}],items,{organisationRefs:['abc'],branchRefs:['abc-kochi'],organisationName:'ABC Technologies Pvt Ltd',branchName:'Kochi Branch'}),'');
 assert.match(orderScopeError([{item:'item-2'},{item:'item-3'}],items,{organisationRefs:['abc'],organisationName:'ABC Technologies Pvt Ltd'}),/Imported panel is not set up for/);
